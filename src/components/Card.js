...
export default function Card(props) {
  ...
  const { name, color, price,size, link, onClickPlus, onClickFavorite} = props;
  
  const[isAdded,setIsAdded]=useState(false)

  const onClickAdd = ()=>{
    !isAdded ? onClickPlus(props) : console.log('s');
    setIsAdded(!isAdded)
  }
  ...
  return (
    <div className="card">
        <img onClick={onClickLike} className="favorite" src={!isFavorit? "./img/unliked.svg":"./img/liked.svg"} />
      ...
      <div className="card-bottom">
        ...
        <div className="Added" onClick={onClickAdd}>{!isAdded?svgPlus:svgChecked
}</div>
      </div>
    </div>
  );
