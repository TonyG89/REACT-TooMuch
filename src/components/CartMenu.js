  ...
  const onClickOrder = async () => {
    try {
      ...
      for (let index = 0; index < cartClothes.length; index++) {
        ...
        await axios.delete(
          `https://630927d6722029d9dddf3c35.mockapi.io/cart/${item.id}`
        );
        await delay(1000);
      }
    } catch (error) {
        ...
        <div className="cart-menu-top">
          ...
          {items.length > 0 ? (
            ...
            <Info
              ...
              title={
                orderComplete ? "Ваше замовлення оформлене" : "Кошик пустий"
              }
              description={
                orderComplete
                  ? `Ваше замовлення №${orderId}. З вами в найближчий час зв'яжуться.`
